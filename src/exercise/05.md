...
- You can add styles in React components in two ways:
  - Inline Styles:
  ...
  In terms of React, `{{}}` is a combination of JSX expression and an object expression. These properties are `camelCased` rather than `kebab-cased`. This matches the style property of DOM nodes which is a `CSSStyleDeclaration` object.
  - Adding the `className` property: We use the `className` property because `class` is a reserved keyword in JavaScript.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=05%3A%20Styling&em=bora.shivayan%40gmail.com).
