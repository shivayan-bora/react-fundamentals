...
4. In this case, we are managing the input field with a local state using the `useState` hook to make it into a controlled input field. for that we need to provide a `value` tied to the state and an `onChange` handler to update the state.
...
7. We can also capture the input field value if we tie it to a `ref` using the `useRef` Hook.
8. In React, we have `SyntheticEvents` whose instances are passed onto every event handlers. The are wrappers around the native browser event handlers and behave the exact same.
9. If you want to access the native browser events, you can do that by using `nativeEvent` attribute.
10. This was done in order to improve performance.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=06%3A%20Forms&em=bora.shivayan%40gmail.com).
