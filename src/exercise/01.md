...
First we need to select the root node in the HTML:

```js
const root = document.querySelector('#root');
```

Then we can create a new `<div>` element:

```js
const div = document.createElement('div');
```

Then we can add a text node to the `<div>` element:
...
Finally we can append the `<div>` element to the root node:
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=01%3A%20Basic%20JavaScript-rendered%20Hello%20World&em=bora.shivayan%40gmail.com).
