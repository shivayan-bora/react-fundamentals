...
1. You can create custom components that can be reused anywhere in your application.
2. Components can effectively return only a single element, but they can have multiple components nested inside them.
3. We can use `PropTypes` for `prop` validation.
4. `PropType` validation results in some runtime overhead resulting in suboptimal performance and hence it's not run in production.
5. You can use React fragments to group multiple children together but you don't want to add an extra DOM node for that.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=04%3A%20Creating%20custom%20components&em=bora.shivayan%40gmail.com).
