...
So far we've been using JavaScript to generate HTML. For this one we would need two packages, React and ReactDOM.

React is a library that allows us to generate HTML. ReactDOM is a library that allows us to render React components to the DOM. ReactDOM is specifically used to render React components to the browser DOM.

To create a React component, we need to use React.createElement to create a React component.
...
1. The tag name of the element
2. An object of attributes (called props)
3. The content of the element (called children)

We can now use ReactDOM.render to render the element to the DOM.

```js
  ReactDOM.render(element, document.getElementById('root'));
```

In case of React 18, we need to render the component over the root node in the following manner:
...
The role of ReactDOM.render is to take a React element and render it to the DOM.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=02%3A%20Intro%20to%20raw%20React%20APIs&em=bora.shivayan%40gmail.com).
